...
class generator(object):
    ...
    def QAM_N_ind(self):
        ...
        if self.mod_scheme == 'MIX':
            ...
            indices_QAM4 = tf.random_uniform(
                ...
                maxval=2,
                dtype=tf.int32)
            indices_QAM = mapping * tf.concat([indices_QAM64, tf.gather([0, 2, 5, 7], indices_QAM16), indices_QAM4 * 7],
                                              axis=2)
    ...
    def random_indices(self):
        ...
        if self.mod_scheme == 'QAM':
            return self.QAM_N_ind()
        elif self.mod_scheme == 'MIX':
            return self.QAM_N_ind()
        else:
            print("Modulation is not supported")
    ...
    def channel(self, x, snr_db_min, snr_db_max, H, dataset_flag, correlation_flag, use_correlated_H=False):
        ...
        if self.mod_scheme == 'MIX':
            ...
            powQAM4 = 10. * tf.log((tf.square(self.constellation[0]) + tf.square(self.constellation[7])) / 2.) / tf.log(
                10.)
            powQAM16 = 10. * tf.log((self.constellation[0] ** 2 + self.constellation[2] ** 2 + self.constellation[
                5] ** 2 + self.constellation[7] ** 2) / 4.) / tf.log(10.)
        ...
        complexnoise_sigma = tf.pow(10., (10. * np.log10(self.NT) + self.Hdataset_powerdB - snr_db - 10. * np.log10(
            self.NR)) / 20.)  # tf.pow(10., -snr_db/20.)
        # Channel Output
