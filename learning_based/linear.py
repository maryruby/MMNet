...
class linear(object):
    ...
    def MMNet(self, shatt, rt, features):
        ...
        Wr = tf.Variable(tf.random_normal(shape=[1, self.NT // 2, self.NR // 2], mean=0., stddev=0.01), name='Wr')
        Wi = tf.Variable(tf.random_normal([1, self.NT // 2, self.NR // 2], 0., 0.01), name='Wi')
        W = tf.concat([tf.concat([Wr, -Wi], axis=2), tf.concat([Wi, Wr], axis=2)], axis=1, name='W')
        W = tf.tile(W, [batch_size, 1, 1], name='W')
        H = features['H']
    ...
    def lin_DetNet(self, shatt, rt, features):
        ...
        zt = shatt + batch_matvec_mul(tf.matmul(tf.tile(Theta1, [batch_size, 1, 1]), W),
                                      features['y'] - batch_matvec_mul(H, shatt)) + theta3
    ...
    def OAMPNet(self, shatt, rt, features):
        ...
        v2_t = tf.divide((tf.reduce_sum(tf.square(rt), axis=1, keep_dims=True) - self.NR * tf.square(noise_sigma) / 2.),
