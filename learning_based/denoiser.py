...
class denoiser(object):
    ...
    def MMNet(self, zt, xhatt, rt, features, linear_helper):        
        ...
        v2_t = tf.expand_dims(v2_t, axis=2) #квадратные скобки, деленная на квадрат фр.нормы Н
        C_t = tf.eye(self.NT, batch_shape=[tf.shape(H)[0]]) - tf.matmul(W_t, H) #числитель
        tau2_t = 1./self.NT * tf.reshape(tf.trace(tf.matmul(C_t, C_t, transpose_b=True)), [-1,1,1]) * v2_t \
                 + tf.square(tf.reshape(noise_sigma,[-1,1,1])) / (2.*self.NT) * tf.reshape(tf.trace(tf.matmul(W_t, W_t, transpose_b=True)), [-1,1,1])
        shatt1, _ = self.gaussian(zt, {'tau2_t':tau2_t/tf.Variable(tf.random_normal([1,self.NT,1], 1., 0.1))})
        denoiser_helper = {'onsager':tf.Variable(0., trainable=False)}
        return shatt1, denoiser_helper, shatt1

    def MMNet_ideal(self, zt, xhatt, rt, features, linear_helper):
        H = features['H']
        noise_sigma = features['noise_sigma']
        W_t = linear_helper['W']
        HTH = tf.matmul(H, H, transpose_a=True)
        v2_t = tf.divide(
            (tf.reduce_sum(tf.square(rt), axis=1, keep_dims=True) - self.NR * tf.square(noise_sigma) / 2.),
            tf.expand_dims(tf.trace(HTH), axis=1))
        v2_t = tf.maximum(v2_t, 1e-9)
        v2_t = tf.expand_dims(v2_t, axis=2)  # квадратные скобки, деленная на квадрат фр.нормы Н
        C_t = tf.eye(self.NT, batch_shape=[tf.shape(H)[0]]) - tf.matmul(W_t, H)  # числитель
        tau2_t = tf.reshape(tf.trace(tf.matmul(C_t, C_t, transpose_b=True)), [-1, 1, 1]) * v2_t \
                 + tf.square(tf.reshape(noise_sigma, [-1, 1, 1])) / 2.
        theta_1 = tf.Variable(tf.random_normal([1, self.NT, 1], 1., 0.1))
        shatt1, _ = self.gaussian(zt, {'tau2_t': theta_1 * tau2_t / self.NT})
        denoiser_helper = {'onsager': tf.Variable(0., trainable=False)}
        return shatt1, denoiser_helper, shatt1
    ...
    def featurous_nn_simplified(self, zt, xhatt, rt, features, linear_helper):
        H = features['H']
        with tf.name_scope("denoiser_featurous_nn"):
            feature1 = tf.reduce_sum(tf.square(rt), axis=1, keep_dims=True)
            feature2 = tf.trace(tf.matmul(H,H, transpose_a=True))
            feature2 = tf.reshape(feature2, [-1,1])
            feature = tf.concat([feature1, feature2], axis=1)
            tau2_t = fc_layer(feature, 10, layer_name="fc1", act=tf.nn.relu)
            tau2_t = fc_layer(tau2_t, 4, layer_name="fc2", act=tf.nn.relu)
    ...
    def featurous_nn_sigma(self, zt, xhatt, rt, features, linear_helper):
        H = features['H']
        noise_sigma = features['noise_sigma']
        with tf.name_scope("denoiser_featurous_nn"):
            feature1 = tf.reduce_sum(tf.square(rt), axis=1, keep_dims=True)
            feature2 = 1./tf.trace(tf.matmul(H,H, transpose_a=True))  # TODO: remove 1./
            feature2 = tf.reshape(feature2, [-1,1])
            feature3 = tf.reshape(noise_sigma, shape=(-1,1))
            feature = tf.concat([feature1, feature2, feature3], axis=1)
            tau2_t = fc_layer(feature, 10, layer_name="fc1", act=tf.nn.relu)
            tau2_t = fc_layer(tau2_t, 4, layer_name="fc2", act=tf.nn.relu)
            tau2_t = fc_layer(tau2_t, 1, layer_name="fc3", act=tf.square)
            tau2_t = tf.expand_dims(tau2_t, axis=2)
